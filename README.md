...
> Discussed in [[2026-07-07]] → Discussed in **Tuesday, Jul 7, 2026**

Strictly cosmetic. The plugin never renames files, never writes frontmatter or aliases, never changes note content. Turn it off and everything is exactly as it was.
...
Link text is where readability matters most anyway: that's where dates appear mid-sentence in your prose. If Obsidian ships a public display-name API for the other contexts, extending the plugin there becomes a clean follow-up.

## Install

Until it's in the community directory: download `main.js` and `manifest.json` from the latest release into `<vault>/.obsidian/plugins/readable-dates/`, then enable it in **Settings → Community plugins**. Or install via [BRAT](https://github.com/TfTHacker/obsidian42-brat).
